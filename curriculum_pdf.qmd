...
Computational scientist and scientific software developer with {years}+  experience in high-performance computing, large-scale numerical simulations and machine learning. 
...
\begin{large}
  \begin{itemize}
    ...
    \item \textbf{Improved algorithm scaling from $O(N^6)$ to $O(N \log N)$} through FFTs and convolutions
    \item \textbf{Built and maintained open-source simulation libraries} (Python/C++) adopted for Monte Carlo simulations and data analysis
...
\begin{large}
  \begin{itemize}
    \item \textbf{Optimization of GPU kernels (CUDA)} achieving a $\sim$1.5$\times$ throughput improvement via auto-tuning of Multigrid solver parameters on HPC clusters
...
\begin{large}
  \begin{itemize}
    \item Built a full-stack \href{https://streamlit.io/}{Streamlit} web application for a nutritionist's practice to manage patient records, appointment history, and nutritional assessments. Delivered solo from requirements to deployment
  \end{itemize}
\end{large}
